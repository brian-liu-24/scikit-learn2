...
affected_doc_paths() {
    ...
    sklearn1_files=$(echo "$files" | grep '^sklearn1/')
    if [ -n "$sklearn1_files" ]
    then
        grep -hlR -f<(echo "$sklearn1_files" | sed 's/^/scikit-learn\/blob\/[a-z0-9]*\//') doc/_build/html/stable/modules/generated | cut -d/ -f5-
    fi
}
