...
# We ignore files from sklearn1/externals. Unfortunately there is no
...
# ':!sklearn1/externals' but it is only available on git 1.9 and Travis
# uses git 1.8.
# We need the following command to exit with 0 hence the echo in case
# there is no match
MODIFIED_FILES="$(git diff --name-only $COMMIT_RANGE | grep -v 'sklearn1/externals' | \
...
if [[ "$MODIFIED_FILES" == "no_match" ]]; then
    echo "No file outside sklearn1/externals and doc/sphinxext has been modified"
else
    ...
    flake8 --exclude=sklearn1/externals/ --select=F401 sklearn1/ examples/
fi
