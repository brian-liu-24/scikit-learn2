...
"""
The :mod:`sklearn1.naive_bayes` module implements Naive Bayes algorithms. These
...
class ComplementNB(_BaseDiscreteNB):
    ...
    >>> X = rng.randint(5, size=(6, 100))
    >>> y = np.array([1, 2, 3, 4, 5, 6])
    >>> from sklearn1.naive_bayes import ComplementNB
    >>> clf = ComplementNB()
    >>> clf.fit(X, y)
    ComplementNB()
    >>> print(clf.predict(X[2:3]))
...
class BernoulliNB(_BaseDiscreteNB):
    ...
    >>> from sklearn1.naive_bayes import BernoulliNB
    >>> clf = BernoulliNB()
    >>> clf.fit(X, Y)
    BernoulliNB()
    >>> print(clf.predict(X[2:3]))
