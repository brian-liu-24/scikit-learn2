...
from sklearn1 import __version__
from sklearn1.utils._openmp_helpers import _openmp_parallelism_enabled


def test_openmp_parallelism_enabled():
    # Check that sklearn1 is built with OpenMP-based parallelism enabled.
    # This test can be skipped by setting the environment variable
    # ``sklearn1_SKIP_OPENMP_TEST``.
    if os.getenv("sklearn1_SKIP_OPENMP_TEST"):
        pytest.skip("test explicitly skipped (sklearn1_SKIP_OPENMP_TEST)")
    ...
    err_msg = textwrap.dedent(
        ...
        sklearn1_SKIP_OPENMP_TEST to any value.
        """).format(base_url)
