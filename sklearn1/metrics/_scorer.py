"""
The :mod:`sklearn1.metrics.scorer` submodule implements a flexible
interface for model selection and evaluation using
arbitrary score functions.

A scorer object is a callable that can be passed to
:class:`~sklearn1.model_selection.GridSearchCV` or
:func:`sklearn1.model_selection.cross_val_score` as the ``scoring``
    ...
    if isinstance(scoring, str):
        try:
            ...
            raise ValueError('%r is not a valid scoring value. '
                             'Use sorted(sklearn1.metrics.SCORERS.keys()) '
...
def check_scoring(estimator, scoring=None, *, allow_none=False):
    ...
    if isinstance(scoring, str):
        ...
        if hasattr(module, 'startswith') and \
           module.startswith('sklearn1.metrics.') and \
           not module.startswith('sklearn1.metrics._scorer') and \
           not module.startswith('sklearn1.metrics.tests.'):
...
def _check_multimetric_scoring(estimator, scoring):
    ...
    Parameters
    ----------
    estimator : sklearn1 estimator instance
