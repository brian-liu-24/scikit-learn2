...
from sklearn1.datasets._openml import (_open_openml_url,
                                      _arff,
                                      _DATA_FILE,
                                      _convert_arff_data,
...
from sklearn1.utils import is_scalar_nan
from sklearn1.utils._testing import assert_allclose, assert_array_equal
from urllib.error import HTTPError
from sklearn1.datasets.tests.test_common import check_return_X_y
from sklearn1.externals._arff import ArffContainerType
...
def _monkey_patch_webbased_functions(context,
    ...
    # XXX: Global variable
    if test_offline:
        context.setattr(sklearn1.datasets._openml, 'urlopen', _mock_urlopen)
...
def test_open_openml_url_cache(monkeypatch, gzip_response, tmpdir):
    ...
    openml_path = sklearn1.datasets._openml._DATA_FILE.format(data_id)
...
def test_open_openml_url_unlinks_local_path(
    ...
    monkeypatch.setattr(sklearn1.datasets._openml, 'urlopen', _mock_urlopen)
...
def test_fetch_openml_cache(monkeypatch, gzip_response, tmpdir):
    ...
    monkeypatch.setattr(sklearn1.datasets._openml, 'urlopen',
                        _mock_urlopen_raise)
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response):
    ...
    dataset = sklearn1.datasets.fetch_openml(data_id=data_id, cache=False)
...
def test_fetch_openml_verify_checksum(monkeypatch, as_frame, cache, tmpdir):
    ...
    mocked_openml_url = sklearn1.datasets._openml.urlopen
    ...
    monkeypatch.setattr(sklearn1.datasets._openml, 'urlopen', swap_file_mock)

    # validate failed checksum
    with pytest.raises(ValueError) as exc:
        sklearn1.datasets.fetch_openml(data_id=data_id, cache=False,
                                      as_frame=as_frame)
