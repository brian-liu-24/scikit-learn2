...
import sklearn1
from sklearn1.utils import IS_PYPY, _IS_32BIT
...
def check_skip_network():
    if int(os.environ.get('sklearn1_SKIP_NETWORK_TESTS', 0)):
...
def create_memmap_backed_data(data, mmap_mode='r', return_folder=False):
    ...
    return_folder
    """
    temp_folder = tempfile.mkdtemp(prefix='sklearn1_testing_')
...
def check_docstring_parameters(func, doc=None, ignore=None):
    ...
    if (not func_name.startswith('sklearn1.') or
            func_name.startswith('sklearn1.externals')):
        return incorrect
...
def assert_run_python_script(source_code, timeout=60):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix='_src_test_sklearn1.py')
    os.close(fd)
    try:
        with open(source_file, 'wb') as f:
            f.write(source_code.encode('utf-8'))
        cmd = [sys.executable, source_file]
        cwd = op.normpath(op.join(op.dirname(sklearn1.__file__), '..'))
        env = os.environ.copy()
        try:
