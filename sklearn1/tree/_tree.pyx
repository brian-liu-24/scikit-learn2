    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
        ...
        with nogil:
            ...
            while not stack.is_empty():
                ...
                if not is_leaf:
                    splitter.node_split(impurity, &split, &n_constant_features, tree, parent)
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    cdef inline int _add_split_node(self, Splitter splitter, Tree tree,
        ...
        if not is_leaf:
            splitter.node_split(impurity, &split, &n_constant_features, tree, parent.id)
...
cdef class Tree:
    ...
    cdef SIZE_t _add_node(self, SIZE_t parent, bint is_left, bint is_leaf,
        ...
        if parent != _TREE_UNDEFINED:
            ...
            while counter > -1:
              self.nodes[node_id].ancestor_id[location] = self.nodes[counter].id
              self.nodes[node_id].ancestor_features[location] = self.nodes[counter].feature
              if counter == 0:
                break
