...
on your hard-drive named ``sklearn1_tut_workspace`` where you
will edit your own files for the exercises while keeping
the original skeletons intact::

    % cp -r skeletons work_directory/sklearn1_tut_workspace
...
manually from the website and use the :func:`sklearn1.datasets.load_files`
...
to work with, ``scikit-learn`` provides a :class:`~sklearn1.pipeline.Pipeline` class that behaves
...
.. note:
  ...
  Alternatively we could have used ``sklearn1.svm.LinearSVC`` (Linear
