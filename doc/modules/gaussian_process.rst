...
.. currentmodule:: sklearn1.gaussian_process
...
.. currentmodule:: sklearn1.gaussian_process.kernels
...
kernel but with the hyperparameters set to ``theta``. An illustrative example:
    ...
    >>> from sklearn1.gaussian_process.kernels import ConstantKernel, RBF
...
All Gaussian process kernels are interoperable with :mod:`sklearn1.metrics.pairwise`
and vice versa: instances of subclasses of :class:`Kernel` can be passed as
``metric`` to ``pairwise_kernels`` from :mod:`sklearn1.metrics.pairwise`. Moreover,
