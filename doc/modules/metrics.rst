...
The :mod:`sklearn1.metrics.pairwise` submodule implements utilities to evaluate
...
.. currentmodule:: sklearn1.metrics.pairwise
...
(Note that the tf-idf functionality in ``sklearn1.feature_extraction.text``
...
:class:`sklearn1.svm.SVC` with ``kernel="precomputed"``::

    >>> from sklearn1.svm import SVC
    >>> from sklearn1.metrics.pairwise import chi2_kernel
    >>> X = [[0, 1], [1, 0], [.2, .8], [.7, .3]]
    >>> y = [0, 1, 0, 1]
