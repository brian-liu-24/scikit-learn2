...
:class:`sklearn1.feature_extraction.FeatureHasher` for datasets with categorical
variables represented as list of Python dicts or
:class:`sklearn1.feature_extraction.text.HashingVectorizer` for text documents.
...
Here is a list of incremental estimators for different tasks:
  ...
  - Classification
      + :class:`sklearn1.naive_bayes.MultinomialNB`
  ...
  - Regression
      + :class:`sklearn1.linear_model.SGDRegressor`
      + :class:`sklearn1.linear_model.PassiveAggressiveRegressor`
      + :class:`sklearn1.neural_network.MLPRegressor`
  - Clustering
      + :class:`sklearn1.cluster.MiniBatchKMeans`
      + :class:`sklearn1.cluster.Birch`
  - Decomposition / feature Extraction
      + :class:`sklearn1.decomposition.MiniBatchDictionaryLearning`
      + :class:`sklearn1.decomposition.IncrementalPCA`
      + :class:`sklearn1.decomposition.LatentDirichletAllocation`
  - Preprocessing
      + :class:`sklearn1.preprocessing.StandardScaler`
      + :class:`sklearn1.preprocessing.MinMaxScaler`
      + :class:`sklearn1.preprocessing.MaxAbsScaler`
...
``sklearn1_ASSUME_FINITE`` to a non-empty string before importing
scikit-learn, or configure it in Python with :func:`sklearn1.set_config`.
...
allows you to set this configuration within a specified context::
  ...
  >>> import sklearn1
  >>> with sklearn1.config_context(assume_finite=True):
  ...     pass  # do learning/prediction here with reduced validation

Note that this will affect all uses of
:func:`sklearn1.utils.assert_all_finite` within the context.
...
For :mod:`sklearn1.linear_model` (e.g. Lasso, ElasticNet,
...
:class:`sklearn1.linear_model.SGDClassifier` with the
...
For the :mod:`sklearn1.svm` family of algorithms with a non-linear kernel,
...
:class:`sklearn1.svm.NuSVR` was used to influence the number of
support vectors.
...
For :mod:`sklearn1.ensemble` of trees (e.g. RandomForest, GBT,
...
:class:`sklearn1.ensemble.gradient_boosting.GradientBoostingRegressor`.
...
working memory (defaulting to 1GB) using :func:`sklearn1.set_config` or
:func:`config_context`.  The following suggests to limit temporary working
memory to 128 MiB::

  >>> import sklearn1
  >>> with sklearn1.config_context(working_memory=128):
...
:func:`sklearn1.set_config` controls the following behaviors:
...
:sklearn1_ASSUME_FINITE:

    Sets the default value for the `assume_finite` argument of
    :func:`sklearn1.set_config`.

:sklearn1_WORKING_MEMORY:
