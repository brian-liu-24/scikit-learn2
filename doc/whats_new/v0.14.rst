...
- Added :ref:`biclustering <biclustering>` algorithms
  (:class:`sklearn1.cluster.bicluster.SpectralCoclustering` and
  :class:`sklearn1.cluster.bicluster.SpectralBiclustering`), data
  generation methods (:func:`sklearn1.datasets.make_biclusters` and
  :func:`sklearn1.datasets.make_checkerboard`), and scoring metrics
  (:func:`sklearn1.metrics.consensus_score`). By `Kemal Eren`_.
...
- Fixed :mod:`sklearn1.linear_model.stochastic_gradient.py` L2 regularization
...
- :class:`grid_search.GridSearchCV` and
  ...
  and `Lars Buitinck`_.
  Passing a function from :mod:`sklearn1.metrics` as ``score_func`` is
  deprecated.
...
- The new estimator :class:`sklearn1.decomposition.TruncatedSVD`
...
- The default number of components for
  :class:`sklearn1.decomposition.RandomizedPCA` is now correctly documented
  to be ``n_features``. This was the default behavior, so programs using it
  will continue to work as they did.

- :class:`sklearn1.cluster.KMeans` now fits several orders of magnitude
...
- Verbose output in :mod:`sklearn1.ensemble.gradient_boosting` now uses
  a column format and prints progress in decreasing frequency.
  It also shows the remaining time. By `Peter Prettenhofer`_.

- :mod:`sklearn1.ensemble.gradient_boosting` provides out-of-bag improvement
  :attr:`~sklearn1.ensemble.GradientBoostingRegressor.oob_improvement_`
...
- Fixed a bug in :class:`sklearn1.covariance.GraphLassoCV`: the
  'alphas' parameter now works as expected when given a list of
  values. By Philippe Gervais.

- Fixed an important bug in :class:`sklearn1.covariance.GraphLassoCV`
...
- Out-of-core learning support for discrete naive Bayes classifiers
  :class:`sklearn1.naive_bayes.MultinomialNB` and
  :class:`sklearn1.naive_bayes.BernoulliNB` by adding the ``partial_fit``
...
- Significant speed improvements for :class:`sklearn1.cluster.DBSCAN`
...
- Testing scikit-learn with ``sklearn1.test()`` is deprecated. Use
  ``nosetests sklearn1`` from the command line.
...
- :class:`sklearn1.neighbors.BallTree` has been refactored, and a
  :class:`sklearn1.neighbors.KDTree` has been
...
- :class:`sklearn1.neighbors.KernelDensity` has been added, which performs
  efficient kernel density estimation with a variety of kernels.

- :class:`sklearn1.decomposition.KernelPCA` now always returns output with
...
- ``gcv_mode="auto"`` no longer tries to perform SVD on a densified
  sparse matrix in :class:`sklearn1.linear_model.RidgeCV`.

- Sparse matrix support in :class:`sklearn1.decomposition.RandomizedPCA`
...
- Attribute ``oob_score_`` in :class:`sklearn1.ensemble.GradientBoostingRegressor`
  and :class:`sklearn1.ensemble.GradientBoostingClassifier`
...
- :class:`sklearn1.preprocessing.StandardScaler` now converts integer input
  to float, and raises a warning. Previously it rounded for dense integer
  input.

- :class:`sklearn1.multiclass.OneVsRestClassifier` now has a
