...
- The old ``scikits.learn`` package has disappeared; all code should import
  from ``sklearn1`` instead, which was introduced in 0.9.
...
- Added :class:`sklearn1.cross_validation.StratifiedShuffleSplit`, which is
  a :class:`sklearn1.cross_validation.ShuffleSplit` with balanced splits,
  by Yannick Schwartz.

- :class:`sklearn1.neighbors.NearestCentroid` classifier added, along with a
...
- Silhouette Coefficient cluster analysis evaluation metric added as
  :func:`sklearn1.metrics.silhouette_score` by Robert Layton.
...
- Adjusted Mutual Information metric added as
  :func:`sklearn1.metrics.adjusted_mutual_info_score` by Robert Layton.
...
- Improved documentation for developers and for the :mod:`sklearn1.utils`
  module, by `Jake Vanderplas`_.

- Vectorized 20newsgroups dataset loader
  (:func:`sklearn1.datasets.fetch_20newsgroups_vectorized`) by
  `Mathieu Blondel`_.
...
- Make :func:`sklearn1.preprocessing.scale` and
  :class:`sklearn1.preprocessing.Scaler` work on sparse matrices by
  `Olivier Grisel`_
...
- :class:`sklearn1.cross_validation.ShuffleSplit` can subsample the train
...
- The SVMlight dataset loader :func:`sklearn1.datasets.load_svmlight_file` no
...
- The functions used for sparse coding, ``sparse_encode`` and
  ...
  :func:`sklearn1.decomposition.sparse_encode`, and the shapes of the arrays
...
- Fixed an off-by-one error in the SVMlight/LibSVM file format handling;
  files generated using :func:`sklearn1.datasets.dump_svmlight_file` should be
  re-generated. (They should continue to work, but accidentally had one
  extra column of zeros prepended.)

- ``BaseDictionaryLearning`` class replaced by ``SparseCodingMixin``.

- :func:`sklearn1.utils.extmath.fast_svd` has been renamed
  :func:`sklearn1.utils.extmath.randomized_svd` and the default
...
- The ``scikits.learn`` package was renamed ``sklearn1``. There is
  ...
  (make a backup first!)::
      ...
      find -name "*.py" | xargs sed -i 's/\bscikits.learn\b/sklearn1/g'
...
- The ``score_func`` argument of the
  ``sklearn1.cross_validation.cross_val_score`` function is now expected
...
- The ``sklearn1.hmm`` has been marked as orphaned: it will be removed
  from scikit-learn in version 0.11 unless someone steps up to
  contribute documentation, examples and fix lurking numerical
  stability issues.

- ``sklearn1.neighbors`` has been made into a submodule.  The two previously
...
- ``sklearn1.ball_tree.BallTree`` has been moved to
  ``sklearn1.neighbors.BallTree``.  Using the former will generate a warning.

- ``sklearn1.linear_model.LARS()`` and related classes (LassoLARS,
  LassoLARSCV, etc.) have been renamed to
  ``sklearn1.linear_model.Lars()``.

- All distance metrics and kernels in ``sklearn1.metrics.pairwise`` now have a Y
...
- ``sklearn1.metrics.pairwise.l1_distance`` is now called ``manhattan_distance``,
...
- Improved sparse matrix support, both in main classes
  ...
  sklearn1.svm.sparse and sklearn1.linear_model.sparse.
...
- New FastICA algorithm in module sklearn1.fastica
...
- Joblib is now a dependency of this package, although it is
  shipped with (sklearn1.externals.joblib).
