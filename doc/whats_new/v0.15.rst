...
- ``nose`` is non-longer a runtime dependency to import ``sklearn1``, only for
...
- The ``img_to_graph`` and ``grid_tograph`` functions in
  :mod:`sklearn1.feature_extraction.image` now return ``np.ndarray``
...
- Speed improvement of the :mod:`sklearn1.ensemble.gradient_boosting` module.
  By `Gilles Louppe`_ and `Peter Prettenhofer`_.

- Various enhancements to the  :mod:`sklearn1.ensemble.gradient_boosting`
...
- Faster :class:`sklearn1.ensemble.ExtraTrees` by caching feature values.
  By `Arnaud Joly`_.
...
- :mod:`sklearn1.hmm` is deprecated. Its removal is planned
